...
class CSVEditorApp:
    ...
    def create_main_screen(self):
        ...
        for name in self.csv_files.keys():
            tk.Radiobutton(
                self.root, text=name, variable=self.selected_file, value=name
            ).pack(anchor="w")
    ...
    def create_edit_screen(self):
        ...
        tk.Button(button_frame, text="編集", command=self.edit_row).pack(
            side=tk.LEFT, padx=5
        )
        tk.Button(button_frame, text="追加", command=self.add_row).pack(
            side=tk.LEFT, padx=5
        )
        tk.Button(button_frame, text="削除", command=self.delete_row).pack(
            side=tk.LEFT, padx=5
        )
        tk.Button(button_frame, text="戻る", command=self.create_main_screen).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def open_edit_window(self, item, values):
        ...
        tk.Button(edit_win, text="適用", command=apply_changes).grid(
            row=len(self.df.columns), columnspan=2, pady=10
        )
